...
application may use the constants and functions in this namespace for
convenience.
...
In C++20 the range returned by :cpp:func:`all()` models a random access view,
and can be used with other utilities of the ranges library.
...
.. note::
   ...
   STL containers, views don't define type aliases etc.

   In C++20 the enumerator ranges implement ``std::ranges::view_interface``, and
   are thus compatible with native views.

For convenience and readability, the associate namespace generated by
the ``EnumECG`` library also contains aliases to the range functions:
